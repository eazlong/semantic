...
class PTBModel(object):
  ...
  def __init__(self, is_training, config, category, class_number=4, inited=False):
    ...
    with tf.variable_scope(category, reuse=inited, initializer=initializer), tf.name_scope(category):
...
class Predictor:
  ...
  def __init__(self, category, label_size, ckpt_path):
  ...
  def __define_models(self):
    #
    self.train_model = PTBModel(is_training=True, config=self._config,
    ...
    # 初始化类
    self.predict_model = PTBModel(is_training=False, config=eval_config, category=self._category,
